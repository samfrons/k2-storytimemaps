...
and an explore mode. Plain static files — no build step.
...
Serve the repo root with any static server — a server (not `file://`) is
needed for video range requests and map tile loading:
...
The nine `.mp4` excerpts in `clips/` are **not** committed — they are
excerpts of other people's documentaries (never commit or publicly deploy
them). See `clips/README.md` for how to regenerate them from the two source
...
Verify against a local server; the single-file original behavior is the spec.
